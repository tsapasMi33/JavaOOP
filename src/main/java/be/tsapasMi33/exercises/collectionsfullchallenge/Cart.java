package be.tsapasMi33.exercises.collectionsfullchallenge;
...
public class Cart {
    ...
    public Cart(CartType type, int days) {
        this.type = type;
        id = lastId++;
        cartDate = LocalDate.now().minusDays(days); // for testing(days)
    ...
    public void printSalesSlip(Map<String,InventoryItem> inventory) {
        ...
        for (var cartItem : products.entrySet()) {
            ...
            double itemizedPriced = (item.getPrice() * qty);
            total += itemizedPriced;
            System.out.printf("\t%s %-10s (%d)@ $%.2f = $%.2f%n",cartItem.getKey(), item.getProduct().name(), qty, item.getPrice(), itemizedPriced);
        }
